...
class TaskDistributionService {
  ...
  generateCurrentSprintMockDistribution() {
    ...
    const taskDistribution = {
      ...
      bugs: 15,  // ✅ Fixed - was Math.round(sprintTaskCount * 0.15) = 6, now matches Azure DevOps
      design: Math.round(sprintTaskCount * 0.20), // 20% design/stories = 8
      others: Math.round(sprintTaskCount * 0.05)  // 5% others = 3
    };

    // Ensure total adds up to sprintTaskCount (adjust tasks to compensate for fixed bug count)
    ...
    // Mock bug classification for Sprint 25 - updated to reflect 15 bugs
    const bugClassification = {
      deployBugs: 5,  // Scaled up proportionally
      prodIssues: 3,  // Scaled up proportionally
      sitBugs: 4,     // Scaled up proportionally
      uatBugs: 3,     // Scaled up proportionally
      unclassified: 0,
      otherBugs: 0,
      totalBugs: taskDistribution.bugs, // Now 15
      classificationRate: 100, // All bugs classified
      classificationBreakdown: {
        'Deploy': { count: 5, percentage: 33.3, bugs: [] },
        'Prod Issues': { count: 3, percentage: 20.0, bugs: [] },
        'SIT': { count: 4, percentage: 26.7, bugs: [] },
        'UAT': { count: 3, percentage: 20.0, bugs: [] }
      },
      environmentBreakdown: {
        'Deploy': { count: 5, percentage: 33.3, bugs: [] },
        'Prod': { count: 3, percentage: 20.0, bugs: [] },
        'SIT': { count: 4, percentage: 26.7, bugs: [] },
        'UAT': { count: 3, percentage: 20.0, bugs: [] }
      }
    };
